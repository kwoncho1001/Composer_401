...
가이드라인 작성 규칙:
1. 어떤 부분이 설계와 다르게 구현되었는지 명확히 지적하세요.
2. 코드를 어떻게 수정해야 하는지 구체적인 방향(또는 코드 스니펫)을 제시하세요.
3. 마크다운 형식을 사용하여 가독성 좋게 작성하세요.
`;
  ...
  const response = await withTimeout(responsePromise, 45000, { text: "가이드를 생성하지 못했습니다." });
